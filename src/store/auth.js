...
export default {
  ...
  actions: {
    async getRegisterData({ commit, dispatch }, payload) {
      const APIkey = "AIzaSyBPETIeJ35NS3gwPaNxP64x4OWKrrOgOWQ";
      const authUrl =
    ...
    async addNewUser({ commit, state }, payload) {
      try {
        const { data } = await axios.post(
          `https://vintage-base-bccdc-default-rtdb.firebaseio.com//user.json?auth=${state.token}`,
          payload
        );
    ...
    async getUser({ commit }, payload) {
      try {
        const { data } = await axios.get(
          "https://vintage-base-bccdc-default-rtdb.firebaseio.com//user.json"
        );
        for (let key in data) {
          console.log(data[key].userId, payload, '<<<')
